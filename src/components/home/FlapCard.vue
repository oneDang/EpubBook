  ...
  <div class="flap-card-wrapper" v-show="flapCardVisible">
    ...
    <div class="book-card" :class="{ 'animation': runBookCardAnimation }" v-show="runBookCardAnimation">
      <div class="book-card-wrapper">
        ...
        <div class="read-btn" @click.stop="showBookDetailWra(data)">{{ $t('home.readNow') }}</div>
      </div>
    </div>
    <!-- 关闭 -->
...
.flap-card-wrapper {
  ...
  .flap-card-bg {
    ...
    background: white;
    transform: scale(0); //移除动画之后的样式
    opacity: 0;

    &.animation {
      animation: flap-card-move .3s ease-in both; //执行完动画之后停留在最后100%的模样
    }
